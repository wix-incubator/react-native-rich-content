...
describe('useWebEditor', () => {
    ...
    beforeEach(() => {
        ...
        jest.mock('../utils/inline-styles.ts', () => ({
            getActiveInlineStyles: jest.fn()
        }));
        
        inlineStylesUtilsMock = require('../utils/inline-styles.ts');
        globalUtilsMock = require('../utils/global-utils.ts');
        useWebEditorMock = require('./useWebEditor').useWebEditor;
    });

    it('should post message on mount', () => {
        renderHook(() => useWebEditorMock([]));
        expect(globalUtilsMock.postWebviewMessage).toBeCalledWith({type: EDITOR_EVENTS.WEB_EDITOR_DID_MOUNT});
    });
